...
<article class="bg-white border border-rg-lightLavender rounded-card overflow-hidden hover:border-rg-midPurple hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 group">
    <a href="{{ route('products.show', ['slug' => $slug]) }}" class="block relative aspect-square overflow-hidden">
        ...
        @endif
        @if($isNew)
            <span class="absolute top-2 right-2 bg-rg-leafGreen text-white text-xs px-2 py-1 rounded-full">Yeni</span>
        @endif
    </a>
    <div class="p-4">
