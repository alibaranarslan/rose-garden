...
@php
    $metaTitle = __('Ödeme');
    $metaDescription = __('Teslimat bilgileri, sipariş notları ve ödeme tercihinizi tek akışta tamamlayarak Rose Garden siparişinizi güvenle oluşturun.');
@endphp
...
@section('content')
    <div class="mb-6 flex flex-col gap-2 md:mb-8">
        ...
        <p class="max-w-2xl text-sm text-rg-grayText dark:text-white/72">{{ __('Bilgiler, teslimat ve ödeme tek akışta ilerler.') }}</p>
    </div>
    @if (session('error'))
