@php
    ...
    $heading = data_get($settings, "title_override.$locale")
        ?: (filled(data_get($homeContent, 'hero_heading')) ? data_get($homeContent, 'hero_heading') : __('Adıyaman’da taze çiçek ve hediyelik bitkiler.'));
    $subheading = data_get($settings, "subtitle_override.$locale")
        ?: (filled(data_get($homeContent, 'hero_subheading')) ? data_get($homeContent, 'hero_subheading') : __('Gerçek ürün fotoğrafları, net fiyatlar ve Adıyaman içi teslimat desteğiyle hızlıca sipariş verin.'));
